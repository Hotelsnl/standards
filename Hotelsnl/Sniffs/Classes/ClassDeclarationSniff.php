 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Hotelsnl_Sniffs_Classes_ClassDeclarationSniff implements PHP_CodeSniffer_Sniff
{
     ...
     * @return void
     */
    private function braces(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($braceLine !== $classLine) {
            $error = 'Opening brace of a %s must be on the SAME line of the definition';
            $phpcsFile->addError($error, $curlyBrace, 'OpenBraceNewLine', $errorData);
            return;
        } else if ($braceLine > ($classLine + 1)) {
            $error = 'Opening brace of a %s must be on the SAME line as the %s declaration; found %s line(s)';
            $data  = array(
        ...
        if ($tokens[($curlyBrace - 1)]['code'] === T_WHITESPACE) {
            ...
            $expected = ($tokens[$stackPtr]['level'] * $this->indent) + 1;
            if ($spaces !== $expected) {
    ...
    {
        $this->braces($phpcsFile, $stackPtr);
        ...
        */
        ...
        if ($nextContent === true) {
            ...
            $error = 'Closing brace of a %s must NOT be followed by a blank line or a closing PHP tag';
            $data  = array($tokens[$stackPtr]['content']);
