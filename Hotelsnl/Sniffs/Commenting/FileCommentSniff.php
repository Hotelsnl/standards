...
class Hotelsnl_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processPackage($errorPos)
    {
        ...
        if ($package !== null) {
            ...
            if (empty($content) === true) {
            ...
            } else if (strpos($content, 'Hotelsnl') === 0) {
                // Package name must not start with Hotelsnl.
                ...
                $this->currentFile->addError($error, $errorPos, 'HotelsnlPackage', $data);
            }
        }
    ...
    /**
     * Author tag must be 'Hotelsnl Pty Ltd <mysource4@squiz.net>'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processCopyrights($errorPos)
    {
        ...
        if ($copyright !== null) {
            ...
            } else if (preg_match('/^([0-9]{4})? (Hotels.nl)$/', $content) === 0) {
    ...
    protected function processLicense($errorPos)
    {
        ...
        if ($license !== null) {
            ...
            if (empty($url) === true && empty($content) === true) {
                ...
                } else if ($url !== 'http://www.hotels.nl/') {
