 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Hotelsnl_Sniffs_Commenting_FunctionCommentSniff implements PHP_CodeSniffer_Sniff
{
        ...
        foreach ($this->commentParser->getThrows() as $i => $throw) {
            ...
            if (empty($exception) === true) {
                ...
                if (strtolower($firstChar) !== $firstChar) {
                    $error = '@throws tag comment must start with a lowercase letter';
            ...
            foreach ($params as $param) {
                ...
                if ($paramComment === '') {
                    ...
                    if (preg_match('|[a-z]|', $firstChar) === 0) {
                        $error = 'Param comment must start with a lowercase letter';
